...
class Kwf_Update_Test extends Kwf_Test_TestCase
{
    ...
    {
        $updates = Kwf_Update::getUpdatesForDir('Kwf/Update/UpdateDir', 50, 110);
        $this->assertEquals(1, count($updates));
        $this->assertTrue($updates[0] instanceof Kwf_Update_UpdateDir_Update_100);

        $updates = Kwf_Update::getUpdatesForDir('Kwf/Update/UpdateDir', 50, 100);
        $this->assertEquals(0, count($updates));

        $updates = Kwf_Update::getUpdatesForDir('Kwf/Update/UpdateDir', 50, 101);
        $this->assertEquals(1, count($updates));

        $updates = Kwf_Update::getUpdatesForDir('Kwf/Update/UpdateDir', 99, 101);
        $this->assertEquals(1, count($updates));

        $updates = Kwf_Update::getUpdatesForDir('Kwf/Update/UpdateDir', 100, 101);
        $this->assertEquals(1, count($updates));

        $updates = Kwf_Update::getUpdatesForDir('Kwf/Update/UpdateDir', 101, 110);
        $this->assertEquals(0, count($updates));
    }

    public function testGetUpdatesForMultipleDir()
    {
        $updates = Kwf_Update::getUpdatesForDir('Kwf/Update/UpdateMultiple', 1, 1100);
