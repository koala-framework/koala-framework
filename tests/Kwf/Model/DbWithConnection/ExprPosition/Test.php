...
class Kwf_Model_DbWithConnection_ExprPosition_Test extends Kwf_Test_TestCase
{
    ...
    public function testExprDirLazyLoad()
    {
        ...
        $this->assertEquals(2, $m->getRow(1)->positionDir);
        $this->assertEquals(1, $m->getRow(2)->positionDir);
        $this->assertEquals(2, $m->getRow(3)->positionDir);
        $this->assertEquals(1, $m->getRow(4)->positionDir);
        $this->assertEquals(3, $m->getRow(5)->positionDir);
        $m->dropTable();
    }
    ...
    public function testExprDirPreload()
    {
        ...
        $s->whereId(1);
        $this->assertEquals(2, $m->getRow($s)->positionDir);

        $s->whereId(2);
        $this->assertEquals(1, $m->getRow($s)->positionDir);
        ...
        $s->whereId(5);
        $this->assertEquals(3, $m->getRow($s)->positionDir);
