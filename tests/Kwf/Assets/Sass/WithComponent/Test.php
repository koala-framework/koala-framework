 ...
 * @group Sass
 */
class Kwf_Assets_Sass_WithComponent_Test extends Kwf_Test_TestCase
{
    public function testIt()
    {
        ...
        $rootComponent = 'Kwf_Assets_Sass_WithComponent_Root_Component';
        ...
        $type = 'Kwf_Assets_Sass_WithComponent:Test';
        $files = $dep->getAssetUrls($type, 'css', 'web', $rootComponent);
        $expected = array(
            "/assets/all/web/$rootComponent/".Kwf_Trl::getInstance()->getTargetLanguage()."/Kwf_Assets_Sass_WithComponent:Test.css?v=".$v,
        );
        $this->assertEquals($expected, $files);

        $c = $loader->getFileContents("all/web/$rootComponent/".Kwf_Trl::getInstance()->getTargetLanguage()."/Kwf_Assets_Sass_WithComponent:Test.css");
        $this->assertContains("body { height: 50px; }", $c['contents']);
        $this->assertContains("body { width: 100px; }", $c['contents']);
