<?p
class Vps_Dao_Row_File extends Vps_Db_Table_Row_Abstra
    ...
    public function getFileSource
        ...
        if (!$this->id) return nul
        return $this->_getUploadDir() . '/' . $this->i
    ...
    public function generateUrl($class, $componentId, $filename, $type, $addRandom = fals
   
        $checksum = md5(Vps_Media_Password::PASSWORD . $componentId . $type
        $extension = $this->extensio
    ...
    private function _recursiveRemoveDirectory($di
   
        if (!file_exists($dir) || !is_dir($dir)) retur
    ...
    //wird von Zend_Db_Table_Row_Abstract vorm löschen aufgeruf
    protected function _delete
   
        $this->deleteFile(
   

    public function uploadFile($filedat
    ...
    public function copyFile($file, $filename, $extensio
    ...
    public function writeFile($contents, $filename, $extensio
