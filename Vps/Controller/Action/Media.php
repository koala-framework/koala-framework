 ...
 * Das Verzeichnis, in dem die Dateien liegen, muss in der config-Datei unte
 * dem Wert "uploads" eingetragen sein
 
 * Die Tabelle vps_uploads muss existieren
 
 * Der Controller muss abgeleitet und die Routen entsprechend eingerichte
 * werden. Der Controller bietet drei Actions an: original, password, cache
 * Alle Actions sind auf den Parameter 'uploadId' angewiesen. Im Zuge desse
 * sollte auch die Zugangsberechtigungen gesetzt werden
 *
class Vps_Controller_Action_Media extends Vps_Controller_Actio
     ...
     * Die Passwörter sind in der Klasse Vps_Media_Password gespeichert un
     * über die statische Methode get() zu holen. Die Method
    ...
    public function passwordAction(
        ...
        if ($type == 'original') 
            // Direkt auf Originaldatei springe
            $this->originalAction()
        } else 
            $this->cacheAction()
     ...
     * Der Dateiname der gecachten Datei kann durch Überschreiben vo
     * $this->_getCacheFilename() geändert werden (falls es zB. zwe
     * gecachte Dateien für eine Originaldatei gibt)
     * Die Verzeichnisse für den Cache werden automatisch angeleg
     * und ggf. gelöscht
     
     * Das Erstellen der Cache-Datei sollte durch Überschreiben vo
    ...
    public function cacheAction(
        ...
        if (!is_file($target)) 
            // Verzeichnisse anlegen, falls nicht existen
            ...
            // Cache-Datei erstelle
            $source = $row->getFileSource()
            try 
                $this->_createCacheFile($source, $target, $this->_getParam('type'))
            } catch (Exception $e) 
                throw new Vps_Controller_Action_Web_Exception($e->getMessage()); // immer 404 auswerfe
            

            // Aufräumen, falls Verzeichnis angelegt wurde und keine Datei erstellt wurde, wieder lösche
     ...
     * @param string Passwort für die Verwendung in der Prüfsumm
     * @return Prüfsumme als strin
     *
     ...
     * @param string Der Pfad zur Originaldate
     * @param string Der Pfad zur gecachten Date
     *
