<?p
class Vpc_Adm

    protected $_d
    protected $_clas

    protected function __construct($class, Zend_Db_Adapter_Pdo_Mysql $d
   
        $this->_class = $clas
        $this->_db = $d
   

    public static function getInstance($componentClas
   
        if (class_exists($componentClass) 
            class_exists('Vpc_Abstract') 
            is_subclass_of($componentClass, 'Vpc_Abstract'
       
            $class = $componentClas
            while ($class != 'Vpc_Abstract')
                $len = strlen(strrchr($class, '_')
                $setupClass = substr($class, 0, -$len) . '_Admin
//                 try
                    if (class_exists($setupClass))
                        return new $setupClass($componentClass, Zend_Registry::get('db')
                   
//                 } catch (Zend_Exception $e)
//                
                $class = get_parent_class($class
           
       
        return nul
   

    public static function getAvailableComponents($path = '
   
        if ($path == '')
            $path = VPS_PATH . '/Vpc
       
        $return = array(
        foreach (new DirectoryIterator($path) as $item)
            if ($item->getFilename() != '.' 
                $item->getFilename() != '..' 
                $item->getFilename() != '.svn' 
                $item->isDir
            
              
                $pathNew = "$path/$item
                $return = array_merge(self::getAvailableComponents($pathNew), $return
              
            } else if (substr($item->getFilename(), -4) == '.php')
              
                $class = str_replace('/', '_', $item->getPath()
                $class = strrchr($class, 'Vpc_'
                $class .= '_' . str_replace('.php', '', $item->getFilename()
                if (class_exists($class) && is_subclass_of($class, 'Vpc_Abstract'))
                    $return[] = $clas
               

           
       
        return $retur
   

    // **************
  
    public static final function getConfig($class, $pageId = null, $componentKey = null, $config = array(
   
        $admin = Vpc_Admin::getInstance($class
        $adminConfig = $admin->getControllerConfig($pageId, $componentKey
        $config = array_merge($config, $adminConfig
        $controllerClass = $admin->getControllerClass(
        $controllerUrl = $admin->getControllerUrl(
        return Vpc_Admin::createConfig($controllerClass, $controllerUrl, $config, $pageId, $componentKey
   

    public static final function createConfig($controllerClass, $controllerUrl, $config = array(), $pageId = null, $componentKey = nul
   
        if (!is_array($config)) { $config = array();
        if (!isset($config['controllerUrl']))
            $config['controllerUrl'] = $controllerUr
       
        if ($pageId)
            if (!isset($config['baseParams']))
                $config['baseParams'] = array(
           
            $config['baseParams']['page_id'] = $pageI
            $config['baseParams']['component_key'] = $componentKe
       
        $return['config'] = $confi
        $return['class'] = $controllerClas
        return $retur
   

    // **************
  
    public function getControllerClass
   
        return 'Vps.Auto.FormPanel
   

    public function getControllerConfig
   
        return array(
   

    public function getControllerUrl($class = nul
   
        if (is_null($class)) $class = $this->_clas
        if (substr($class, -10) == 'Controller')
            $class = substr($class, 0, -10
       
        return '/admin/component/edit/' . $clas
   

    // *************
  
    protected function _getRow($pageId, $componentKe
   
        $tablename = Vpc_Abstract::getSetting($this->_class, 'tablename'
        if ($tablename)
            $table = new $tablename(array('componentClass'=>$this->_class)
            return $table->find($pageId, $componentKey)->current(
       
        return nul
   

    protected function _getRows($pageId, $componentKe
   
        $tablename = Vpc_Abstract::getSetting($this->_class, 'tablename'
        if ($tablename)
            $table = new $tablename(
            $where = arra
                'page_id = ?' => $pageI
                'component_key = ?' => $componentK
            
            return $table->fetchAll($where
       
        return array(
   

    public function setup() 

    public function delete($pageId, $componentKe
   
        $row = $this->_getRow($pageId, $componentKey
        if ($row)
            $row->delete(
       
   

    public function duplicate($component) 

    function createFormTable($tablename, $field
   
        if (!$this->_tableExists($tablename))
            $f = array(
            $f['page_id'] = 'int(10) unsigned NOT NULL
            $f['component_key'] = 'varchar(255) NOT NULL
            $f = array_merge($f, $fields

            $sql = "CREATE TABLE `$tablename` (
            foreach ($f as $field => $data)
                $sql .= " `$field` $data,"
           
            $sql .= 'PRIMARY KEY (page_id, component_key))
            $sql .= 'ENGINE=InnoDB DEFAULT CHARSET=utf8
            $this->_db->query($sql

            if (isset($fields['vps_upload_id']))
                $this->_db->query("ALTER TABLE $tablena
                    ADD INDEX (vps_upload_id)"
                $this->_db->query("ALTER TABLE $tablena
                    ADD FOREIGN KEY (vps_upload_i
                    REFERENCES vps_uploads (i
                    ON DELETE RESTRICT ON UPDATE RESTRICT"
           
            return tru
       
        return fals
   

    protected function _tableExists($tablenam
   
        return in_array($tablename, $this->_db->listTables()
   

    public static function getComponentFile($class, $filename = '', $ext = 'php', $returnClass = fals
   
        $ret = nul
        $retClass = nul
        while (!$ret && $class != '')
            $curClass = $clas
            if ($filename != '')
                $curClass = substr($class, 0, strrpos($class, '_') + 1) . $filenam
           
            $file = str_replace('_', DIRECTORY_SEPARATOR, $curClass) . '.' . $ex
            $dirs = explode(PATH_SEPARATOR, get_include_path()
            foreach ($dirs as $dir)
                if ($dir == '.') { $dir = getcwd();
                $path = $dir . '/' . $fil
                if (is_file($path))
                    $ret = $pat
                    $retClass = $curClas
                    brea
               
           
            $class = get_parent_class($class
       
        return $returnClass ? $retClass : $re
   

