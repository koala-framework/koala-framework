...
class Vpc_Menu_BreadCrumbs_Admin extends Vpc_Menu_Admin
{
    public function onRowUpdate($row)
    {
        ...
        //bisscen anders als in Menu_Admin, showInMenu nicht ber�cksichtigen sowie
        //nur in Update
        foreach (Vpc_Abstract::getComponentClasses() as $componentClass) {
            foreach (Vpc_Abstract::getSetting($componentClass, 'generators') as $generator) {
                ...
                {
                    if ($row instanceof Vps_Db_Table_Row && is_instance_of(get_class($row->getTable()), $generator['table'])) {
