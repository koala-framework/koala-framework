...
class Vpc_Formular_Captcha_Component extends Vpc_Formular_Field_Abstrac
    ...
    /*
     * Generiert 5 Zufallswert
     *
    private function _generateValue(
    
        $num_chars = 5
        // define characters of which the captcha can consis
        ...
        for($i=1; $i<=$num_chars; $i++)
            // choose randomly a character from alphabet and append it to strin
            $chosen = rand(1, $max )
    ...
    public function generateImage($char_seq 
        ...
        // generate background of randomly built ellipse
        for ($i=1; $i<=200; $i++) 
            $r = round(rand(0, 100 ) )
            $g = round(rand(0, 100 ) )
            $b = round(rand(0, 110 ) )
        ...
        // set each letter with random angle, size and colo
