...
class Vpc_Formular_Captcha_Index extends Vpc_Abstract
{
    private $path = "";
	var $width       = 142; 
	var $height      = 40; 
    ...
    function getTemplateVars($mode)
    {
        ...
        if (isset ($_GET['showPic'])){
	        ...
	        $img = $this->generateImage($path);
            header ("Content-type: image/png");
            imagepng($img);
            imagedestroy($img);
            die();
        } else {
            $path = "http://vps.lorenz.vivid/component/show/Vpc_Formular_Captcha_Index/1/?showPic=";
     ...
     * generiert ein Bild
     */
    function generateImage($char_seq )	
    {
    ...
    function myDecryption ($newString)
    {
      $value = "";
      $check = '';
      $cnt = 0;
