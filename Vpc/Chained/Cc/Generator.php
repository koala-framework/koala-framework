<?php
class Vpc_Chained_Cc_Generator extends Vps_Component_Generator_Abstract
{
    private $_rows = array();
    ...
    protected function _getRows($ids)
    {
        ...
        if ($idsToLoad) {
            ...
            $visible = array();
            Vps_Benchmark::count('DirectoryCcGenerator getRows', count($idsToLoad));
    ...
    public function getChildData($parentDatas, $select = array())
    {
        ...
        foreach ($parentDatas as $parentData) {
            foreach ($this->_getChainedChildComponents($parentData, $select) as $component) {
                ...
                if (!$parentData) {
                    if (!$slaveData) {
                        foreach (Vps_Component_Data_Root::getInstance()->getComponentsByClass('Vpc_Ingenieurbueros_Root_Chained_Component') as $d) {
                            $chainedComponent = Vpc_Chained_Cc_Component::getChainedByMaster($component->parent, $d, $parentDataSelect);
                            if ($chainedComponent) $pData[] = $chainedComponent;
                        }
                    } else {
                        $chainedComponent = Vpc_Chained_Cc_Component::getChainedByMaster($component->parent, $slaveData, $parentDataSelect);
    ...
    public function getGeneratorFlags()
    {
        ...
        if (is_instance_of($this->_class, 'Vpc_Ingenieurbueros_Root_Chained_Component')) {
            $ret['trlBase'] = true;
        }
        return $ret;
    }
