...
Vpc.Advanced.GoogleMap.prototype = {
    ...
    show : function()
    {
        ...
        this.map.setCenter(
            new GLatLng(
                parseFloat(this.options.longitude),
    ...
    activateMarker : function () {
        this.marker = new GMarker(
            new GLatLng(parseFloat(this.options.longitude),
            parseFloat(this.options.latitude)),
            { draggable: false }
        );
    ...
    useFrom : function(Placemark, rewriteInput) {
        ...
        this.mapDir.load(
            'from: ' + pos + ' to: ' + this.options.longitude + ', ' + this.options.latitude,
            { 'locale': 'de_AT' }
        );
        if (rewriteInput) {
            mapContainer.down("form.fromAddress").set({ value: Placemark.address });
        }
