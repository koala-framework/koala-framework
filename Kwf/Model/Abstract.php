...
abstract class Kwf_Model_Abstract implements Kwf_Model_Interface
{
     ...
     * @return array Info how many rows are updated/created/deleted and a mapping key of import array => id of row
     */
    public function syncData(array $data, array $compareColumns, Kwf_Model_Select $select = null)
    {
        $ret = array(
            'check' => 0,
            'update' => 0,
        ...
        foreach ($data as $id => $d) {
            ...
            if ($row) {
                ...
                $ret['create']++;
                $row = $this->createRow($d);
                $row->save();
            }
            $ret['mapping'][$id] = $row->id;
        }
