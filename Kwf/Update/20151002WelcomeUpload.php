    ...
    public function update()
    {
        ...
        foreach ($db->query('SHOW FIELDS FROM kwf_welcome')->fetchAll() as $row) {
            if ($row['Field'] == 'kwf_upload_id' && substr($row['Type'], 0, 3) == 'int') {
                ...
                $db->query('ALTER TABLE  `kwf_welcome` CHANGE  `kwf_upload_id`  `kwf_upload_id` VARBINARY( 36 ) NULL DEFAULT NULL ,
                    CHANGE  `login_kwf_upload_id`  `login_kwf_upload_id` VARBINARY( 36 ) NULL DEFAULT NULL');
                break;
            }
        }
    }
