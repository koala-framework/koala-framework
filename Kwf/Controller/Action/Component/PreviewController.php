...
class Kwf_Controller_Action_Component_PreviewController extends Kwf_Controller_Action
{
    public function indexAction()
    {
        ...
        if (!$this->view->initialUrl) {
            $this->view->initialUrl = 'http://'.$_SERVER['HTTP_HOST'].Kwf_Config::getValue('server.baseUrl').'/';
        }
    }
    ...
    public function redirectAction()
    {
        ...
        if (substr($this->_getParam('url'), 0, strlen(Kwf_Config::getValue('server.baseUrl'))+1) !== Kwf_Config::getValue('server.baseUrl').'/') throw new Kwf_Exception('Invalid Url');
