                    ...
                    if (asyncChunks.indexOf(chunk) == -1) {
                        this.languages.forEach((lang) => {
                            ...
                            if (Object.keys(data).length) {
                                html += 'kwfTrlData.push({source: "web", data: ' + JSON.stringify(data) + '});\n';
                            }

                            data = extractTrlDataFromChunk(chunk, lang, 'kwf');
                            if (Object.keys(data).length) {
                                html += 'kwfTrlData.push({source: "kwf", data: ' + JSON.stringify(data) + '});\n';
                            }
                            if (html) {
                                html = 'window.kwfTrlData = window.kwfTrlData || [];\n' + html;
