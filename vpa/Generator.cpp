                    ...
                    if (boxHash.contains(c2) && c!=c2 && boxHash[c2] == boxHash[c]) {
                        ...
                        if (c->priority() > c2->priority()
                            ...
                            if (!(c2->generator()->generatorFlags & Generator::TypeInherit)) {
                                //TODO: leaked, manchmal muss auch in inherit gel�scht werden (kann vielleicht woanders gel�st werden)
                                //wurde aber eigentlich schon gel�st indem es mit dem generator mitgel�scht wird falls der gel�scht wird
        ...
        case RowUpdated:
            foreach (Generator *g, generators()) {
                if (g->model == model) {
                    ...
                    g->preload(); //tut im moment noch _alles_ preloaden, m�glicherweise nur eine id preloaden wenn zu langsam
                    foreach (ComponentData *d, g->builtComponents) { //das ben�tigt wom�glich einen index wenns zu langsam ist
...
{
    Q_UNUSED(parent); //TODO: könnte verwendet werden um nicht zuviel zu fetchen
...
/*
NICHT MEHR VERWENDET KANN GEL�SCH WERDEN *************************************************
...
QList<ComponentData*> GeneratorLinkTag::build(ComponentData* parent)
{
    ...
    if (!componentIdToComponent.contains(parent->dbId())) {
        //TODO: das d�rfte eigentlich nicht passieren, kann es jedoch wenn eine row
        //noch nie gespeichert wurde; korrekt w�re da die defaultValues zu verwenen
        return QList<ComponentData*>();
    }
