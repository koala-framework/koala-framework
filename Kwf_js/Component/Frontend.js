...
{
    if (config.menu) {
        var menu = new Kwf.Menu.Index(Ext2.DomHelper.insertFirst(document.body, '<div style="position: absolute; z-index:10" \/>', true), {controllerUrl: KWF_BASE_URL+'/admin/menu'});
...
Ext2.extend(Kwf.Component.Frontend.Index, Ext2.util.Observable,
{
    loadComponent : function(data) {
        document.location.href = KWF_BASE_URL+'/admin/pages/?url=' + data.url + '&name=' + data.name;
    }
});
